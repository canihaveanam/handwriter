...
def write_runtime_log(message):
    """记录服务启动、运行过程和异常到 logs/runtime.log。"""
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def preview_image():
    ...
    try:
        ...
        write_runtime_log("========== /preview_image START ==========")
        ...
        write_runtime_log(f"[preview] task_id={task_id}")

        write_runtime_log("[preview] 准备调用 image_generator.iter_images")
...
if __name__ == "__main__":
    write_runtime_log("服务启动：http://0.0.0.0:5000")
